    ...
    def evaluate(self, dimList = None):
        ...
        for n in revDimList:
            if n is revDimList[0]:
                evaOutList = [len(self.evaValues[n-1])*[np.array(self.batchsize*[0])]]
            else:
        ...
        for i,f in enumerate(self.functions):
            ...
            if set(fdims) <= dimSet:   #evaluate function if dependet on given dimensions
                ...
                m = self.mapDimensions(fdims,dimList)
                print(m)
                source = np.array(m.append(evaOut.ndim))-1
                target = np.arange(len(fdims)+1)
